...
def analyze_clusters(df, numeric_columns):
    ...
    # Select variables with correlation > 0.9
    correlated_variables = correlation_matrix[correlation_matrix > 0.9].stack(
    ).reset_index()
    ...
    # Filter variables based on correlation > 0.9
    selected_variables = correlated_variables[correlated_variables['Correlation'] > 0.9]['Variable 1'].unique(
    )
...
def main():
   ...
   # Add cluster labels to the original DataFrame
df['cluster_label'] = clustered_data['cluster']

# Visualize the distribution of clusters
sns.countplot(x='cluster_label', data=df)

# Analyze the characteristics of each cluster
clustered_data['cluster_label'] = df['cluster_label']
cluster_stats = clustered_data.groupby('cluster_label').mean()
...
clustered_data.to_csv('DADOS/clustered_data.csv', index=False)
