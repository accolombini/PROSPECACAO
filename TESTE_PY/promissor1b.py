"""
    ...
    Neste teste trabalhamos com uma variação de 3 sigmas alterando a contagem de zeros subsequentes
...
for record, count in record_counts.items():
    if count > 1:
        ...
        for variable in variables:
            ...
            subsequent_record = teste.iloc[index+1]
            if subsequent_record[variable] == 0:
                print(f"\nRegistro: {record}")
                print("\nVariáveis afetadas:", end=" ")
                print(*variables, sep=", ")
                print("Registro subsequente zerado:")
                print(subsequent_record)
                print()
                break
